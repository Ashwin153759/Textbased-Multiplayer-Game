...
int handleclient(struct client *p, struct client *top) {
    ...
    if (p->state == AWAITING_NAME) {
        ...
        while ((lenName = read(p->fd, buf, 1)) > 0) {
            ...
            if (buf[0] == '\n' || buf[0] == '\r') {
                ...
                p->inputBuffer[p->inputLength] = buf[0];
                p->inputLength++;
            }
    ...
    if (p->state == LOOKING_FOR_MATCH) {
        sprintf(outbuf, "Attempting to find a match...\n");
        write(p->fd, outbuf, strlen(outbuf));
        struct client *other;
        ...
        if (p->opponent == NULL) {
            ...
            sprintf(outbuf, "No match found! please wait...\n");
            write(p->fd, outbuf, strlen(outbuf));
            return 0;
        }
        ...
        if (len > 0){
            if(p->buf[0] == 'a') {
                ...
                if (p->opponent->health <= 0) {
                    ...
                    p->opponent->lastplayed = p; // This is fine here because p->opponent is not NULL yet.
                    p->opponent->opponent = NULL;
                    p->opponent = NULL; // After this line, p->opponent is NULL.
                    p->state = LOOKING_FOR_MATCH;
                    p->opponent->state = LOOKING_FOR_MATCH; // Accessing p->opponent->state after it's set to NULL causes segmentation fault.
                    return 0;
                }
                else {
                    p->state = IN_MATCH_DEFEND;
