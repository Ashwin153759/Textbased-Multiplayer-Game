...
struct client {
    ...
    int health;
    int power_moves;
    int active_mute; // 0 for unmute, 1 for mute
    ...
    if (p->state == IN_MATCH_ATTACK || p->state == IN_MATCH_DEFEND) {
        ...
        if (len > 0){
            if(p->buf[0] == 'a') {
                ...
                if (p->active_mute == 0) {
                    p->active_mute = 1;
                    sprintf(outbuf, "You have muted %s\n", p->opponent->name);
                    write(p->fd, outbuf, strlen(outbuf));
                    sprintf(outbuf, "%s has muted you\n", p->name);
                    write(p->opponent->fd, outbuf, strlen(outbuf));
                    return 0;
                }
                else {
                    p->active_mute = 0;
                    sprintf(outbuf, "You have unmuted %s\n", p->opponent->name);
                    write(p->fd, outbuf, strlen(outbuf));
                    sprintf(outbuf, "%s has unmuted you\n", p->name);
                    write(p->opponent->fd, outbuf, strlen(outbuf));
                    return 0;
                }
            }
